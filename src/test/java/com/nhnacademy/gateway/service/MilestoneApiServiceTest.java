...
class MilestoneApiServiceTest {
    ...
    @BeforeEach
    void setUp() {
        milestoneApiService = new MilestoneApiService(restTemplate, taskApiUrl);
    }
    ...
    void testCreateMilestone() {
        ...
        when(restTemplate.postForEntity(eq(taskApiUrl + "/projects/1/milestones"), eq(request), eq(MilestoneDto.class)))
                .thenReturn(ResponseEntity.ok(response));

        MilestoneDto result = milestoneApiService.createMilestone(1L, request);

        assertEquals("M1", result.name());
        verify(restTemplate).postForEntity(taskApiUrl + "/projects/1/milestones", request, MilestoneDto.class);
    }
    ...
    void testUpdateMilestone() {
        ...
        when(restTemplate.exchange(eq(taskApiUrl + "/projects/1/milestones/2"), eq(HttpMethod.PUT), any(HttpEntity.class), eq(MilestoneDto.class)))
                .thenReturn(ResponseEntity.ok(response));

        MilestoneDto result = milestoneApiService.updateMilestone(1L, 2L, request);

        assertEquals("M2", result.name());
        verify(restTemplate).exchange(eq(taskApiUrl + "/projects/1/milestones/2"), eq(HttpMethod.PUT), any(HttpEntity.class), eq(MilestoneDto.class));
    }

    @Test
    void testDeleteMilestone() {
        milestoneApiService.deleteMilestone(1L, 2L);
        verify(restTemplate).delete(taskApiUrl + "/projects/1/milestones/2");
    }
    ...
    void testGetMilestone() {
        ...
        when(restTemplate.getForEntity(eq(taskApiUrl + "/projects/1/milestones/2"), eq(MilestoneDetailDto.class)))
                .thenReturn(ResponseEntity.ok(response));

        MilestoneDetailDto result = milestoneApiService.getMilestone(1L, 2L);

        assertEquals("M1", result.name());
        verify(restTemplate).getForEntity(taskApiUrl + "/projects/1/milestones/2", MilestoneDetailDto.class);
    }

    @Test
    void testGetMilestones() {
        MilestoneDto[] response = {new MilestoneDto(1L, "M1", LocalDate.now(), LocalDate.now().plusDays(1))};
        when(restTemplate.getForEntity(eq(taskApiUrl + "/projects/1/milestones"), eq(MilestoneDto[].class)))
                .thenReturn(ResponseEntity.ok(response));

        List<MilestoneDto> result = milestoneApiService.getMilestones(1L);

        assertEquals(1, result.size());
        verify(restTemplate).getForEntity(taskApiUrl + "/projects/1/milestones", MilestoneDto[].class);
    }
}
